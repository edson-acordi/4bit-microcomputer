...
# Last Revision:   October, 17, 2025_1
#
# License: GNU GPL v3
#
# Usage: python MikroLeoAsm_year_y.py sourcefilename
# Example: python MikroLeoAsm_year_y.py test.asm
#
...
opCodes = {  # Indirect + Absolute Address (AMODE = 0) - RC:MAddr:LAddr
           ...
           'JPZREG': '0x4E',
           'ADDACCRAMREG': '0x7F',
           # Instruction LDR with bits b13:b12 (MOD) = 0, do nothing
           'NOP': '0x07'
           }
...
if len(sys.argv) != 2:
    print('usage: MikroLeoAsm_20xx_y.py <sourcefile>', '')
    print('Example:', '')
    print('MikroLeoAsm_20xx_y.py test.asm')
    sys.exit(1)
f = open(sys.argv[1], 'r')
...
for i in range(len(lines)):  # PASS 1: do PER LINE replacements
    ...
    # NOP
    lines[i] = re.sub(r'NOP', 'NOP ' + '0', lines[i])
...
# Copyrigh (c) 2022-2025 Edson Junior Acordi
# ------------------------------------------------------------------------------
